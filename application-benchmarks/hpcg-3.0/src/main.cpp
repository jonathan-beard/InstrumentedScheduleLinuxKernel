...
/* START ROI */
	...
	printf("Turn migration REO on? (y/n/d/R (d = turn jumping off, R=realtime prio, s= solely the PIDs of interest))\n");
   char input;
   input = getc(stdin);
   printf ("read %c \n", input);

   if ( (input!='y') && (input!='d')  && (input!='R') && (input!='s') && (input!='n') ){
	   printf("\nim dead\n");
   	exit(-1);
   }
...
unsigned int max_thread_num = 32;
